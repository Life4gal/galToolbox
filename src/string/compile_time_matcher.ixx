export module gal.toolbox.compile_time_matcher;
...
namespace gal::toolbox
{
	...
	template <typename Container>
	using container_iterator_t = std::remove_cv_t<std::remove_reference_t<decltype(std::cbegin(std::declval<Container>()))>>;
	...
	*/
	...
	template <typename Sequence, typename Pattern, typename EqualTo>
	constexpr full_match_result<container_iterator_t<Sequence>, container_iterator_t<Pattern>>
		match(
			Sequence and sequence,
			Pattern and pattern,
	...
	template <typename Pattern, typename EqualTo>
	class compile_time_matcher
	{
	public:
