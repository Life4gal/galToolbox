...
namespace gal::toolbox::container
{
	...
	public:
		...
		public:
			 ...
			 * @return mask
			 */
			[[nodiscard]] constexpr static value_type bit_mask(const size_type pos) noexcept(noexcept(bit_index(std::declval<size_type>())))
			{
			...
			{
				utils::gal_assert(first <= last, "last must greater_equal than first");
				utils::gal_assert(last < bits_of_type, "last should not greater than bits_of_type");
			 ...
			 * @return block after set
			 */
			[[nodiscard]] constexpr static value_type set_block_partial(const value_type block, const size_type first, const size_type last) noexcept(noexcept(set_block_bits(std::declval<value_type>(),
			...
			[[nodiscard]] constexpr static value_type flip_block_partial(
					const value_type block,
					const size_type	 first,
					const size_type	 last) noexcept(noexcept(bit_mask(std::declval<size_type>(), std::declval<size_type>())))
			{
		...
		public:
			...
			{
				utils::gal_assert(index_ + n > index_, "offset overflow");
				return {iterator_, index_ + n};
			}

			[[nodiscard]] constexpr bit_const_iterator operator-(const size_type n) const noexcept
			{
				utils::gal_assert(index_ >= n, "offset underflow");
				return {iterator_, index_ - n};
			}

			[[nodiscard]] constexpr size_type distance(const bit_const_iterator& other) const noexcept
			{
				utils::gal_assert(other.index_ <= index_, "target's offset greater than this");
				return index_ - other.index_;
			}

			[[nodiscard]] constexpr bool operator==(const bit_const_iterator& other) const noexcept
			{
				return index_ == other.index_ and iterator_ == other.iterator_;
			}

			[[nodiscard]] constexpr bool operator not_eq(const bit_const_iterator& other) const noexcept
			{
		...
		public:
			...
			{
				utils::gal_assert(index_ + 1 > index_, "offset overflow");
				auto copy{*this};
				++index_;
				return copy;
			}
			...
			[[nodiscard]] constexpr bit_iterator operator--(int) noexcept
			{
				utils::gal_assert(index_ > 0, "offset underflow");
				auto copy{*this};
				--index_;
				return copy;
			}
			...
			constexpr bit_iterator& operator+=(size_type n) noexcept
			{
				utils::gal_assert(index_ + n > index_, "offset overflow");
				index_ += n;
				return *this;
			...
			[[nodiscard]] constexpr size_type distance(const bit_iterator& other) const noexcept
			{
				utils::gal_assert(other.index_ <= index_, "target's offset greater than this");
				return index_ - other.index_;
			}

			[[nodiscard]] constexpr bool operator==(const bit_iterator& other) const noexcept
			{
				return index_ == other.index_ and iterator_ == other.iterator_;
			}

			[[nodiscard]] constexpr bool operator not_eq(const bit_iterator& other) const noexcept
			{
				return not this->operator==(other);
			}
			...
			[[nodiscard]] constexpr value_type operator*() const noexcept// NOLINT(clang-diagnostic-invalid-constexpr)
			{
				return {*std::next(iterator_, static_cast<std::iter_difference_t<iterator>>(bit_trait::block_index(index_))), bit_trait::bit_index(index_)};
			}

			// let IDE stop prompting us to add explicit for implicit conversion
			// ReSharper disable once CppNonExplicitConversionOperator
			[[nodiscard]] constexpr operator bit_const_iterator() const noexcept
			{
	...
	private:
		 ...
		 */
		[[nodiscard]] constexpr real_reference highest_block() noexcept(noexcept(std::declval<container>().back()))
		{
			utils::gal_assert(size() > 0 && container_size() > 0, "empty container");
		 ...
		 */
		constexpr void init_from_value_type(const size_type size, value_type value) noexcept(
				noexcept(std::declval<container>().resize(calc_blocks_needed(std::declval<size_type>()))))
		{
			utils::gal_assert(container_size() == 0, "empty container");
		...
		{
			utils::gal_assert(pos <= str.size(), "given pos greater than the str 's size");
			utils::gal_assert(n <= size, "container's size cannot less than n");
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& range_operation(
				const size_type pos,
				const size_type len,
				value_type (*partial_block_operation)(value_type, size_type, size_type),
				value_type (*full_block_operation)(value_type))
		{
			utils::gal_assert(pos + len <= total_, "offset greater than the total bits");
			...
			utils::gal_assert(pos + len >= len, "offset overflow");
		...
		{
			utils::gal_assert(container_size() == calc_blocks_needed(total_), "used size not equal the needed size");
	...
	public:
		...
		constexpr explicit basic_dynamic_bitset(// NOLINT(clang-diagnostic-invalid-constexpr)
				...
				const value_type value = {}) noexcept(noexcept(init_from_value_type(std::declval<size_type>(), std::declval<value_type>())))
		{
		...
		{
			utils::gal_assert(check_invariants(), "dtor check_invariants failed");
		}
		 ...
		 */
		constexpr basic_dynamic_bitset(basic_dynamic_bitset&& other) noexcept(std::is_nothrow_move_constructible_v<container>)
			: container_(std::move(other.container_)),
			  total_(other.total_)
		{
			// Required so that check_invariants() works.
			utils::gal_assert((other.container_ = container{}).empty(), "moved container not empty");
			other.total_ = 0;
		}
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& reset(const size_type pos, const size_type len) noexcept(
				noexcept(
						std::declval<basic_dynamic_bitset>().range_operation(
								std::declval<size_type>(),
								std::declval<size_type>(),
								&bit_trait::reset_block_partial,
								&bit_trait::reset_block_full)))
		{
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& reset(const size_type pos) noexcept(
				noexcept(std::declval<container>()[0]))
		{
			utils::gal_assert(pos < total_, "given pos greater than the total bits");
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& set(size_type pos, size_type len, const bool set)
		{
			if (set)
			{
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& set(const size_type pos, const bool set = true) noexcept(
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& flip(const size_type pos, const size_type len) noexcept(
				noexcept(
						std::declval<basic_dynamic_bitset>().range_operation(
								std::declval<size_type>(),
								std::declval<size_type>(),
								&bit_trait::flip_block_partial,
								&bit_trait::flip_block_full)))
		{
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& flip(const size_type pos) noexcept(
				noexcept(std::declval<container>()[0]))
		{
			utils::gal_assert(pos < total_, "given pos greater than the total bits");
		 ...
		 * @return tested value
		 */
		[[nodiscard]] constexpr bool test_and_set(const size_type pos, const bool value) noexcept(
						...
						noexcept(std::declval<container>()[0]))
		{
			...
			if (const auto all_one = ~static_cast<value_type>(0);
				extra == 0)
			{
				for (size_type i = 0; i < container_size(); ++i)
				{
		 ...
		 * @return ref
		 */
		[[nodiscard]] constexpr reference operator[](const size_type index) noexcept(std::is_nothrow_constructible_v<reference, real_reference, size_type>)
		{
		 ...
		 * @return iterator
		 */
		[[nodiscard]] constexpr iterator begin() noexcept(std::is_nothrow_constructible_v<iterator, real_iterator, size_type>)
		{
		 ...
		 * @return iterator
		 */
		[[nodiscard]] constexpr iterator end() noexcept(std::is_nothrow_constructible_v<iterator, real_iterator, size_type>)
		{
		...
		constexpr void resize(const size_type size, const bool value = false)
		{
			...
			if (value and (size > total_))
			{
				...
				{
					utils::gal_assert(old_size >= 1 && old_size <= container_.size(), "impossible happened: old_size < 1 or old_size > current size");
		 ...
		 */
		constexpr void push_back(const bool value) noexcept(
		...
		constexpr void append(const value_type value)
		{
			...
			total_ += bits_of_type;
			utils::gal_assert(check_invariants(), "check_invariants failed");
		}
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& operator and_eq(const basic_dynamic_bitset& other) noexcept(
				noexcept(std::declval<container>()[0]))
		{
			utils::gal_assert(size() == other.size(), "container's size not equal to other's");
			for (size_type i = 0; i < container_size(); ++i)
			{
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& operator or_eq(const basic_dynamic_bitset& other) noexcept(
				noexcept(std::declval<container>()[0]))
		{
			utils::gal_assert(size() == other.size(), "container's size not equal to other's");
			for (size_type i = 0; i < container_size(); ++i)
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& operator xor_eq(const basic_dynamic_bitset& other) noexcept(
				noexcept(std::declval<container>()[0]))
		{
			utils::gal_assert(size() == other.size(), "container's size not equal to other's");
			for (size_type i = 0; i < container_size(); ++i)
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& operator>>=(const size_type n) noexcept(noexcept(operator<<=(std::declval<size_type>())))
		{
			if (n >= total_)
			{
				return reset();
		 ...
		 * @return result
		 */
		[[nodiscard]] constexpr bool operator not_eq(const basic_dynamic_bitset& other) const noexcept
		{
		...
		[[nodiscard]] constexpr bool operator<=(const basic_dynamic_bitset& other) const
				noexcept(
						noexcept(std::declval<basic_dynamic_bitset>().operator<(std::declval<const basic_dynamic_bitset&>())))
		{
		 ...
		 */
		[[nodiscard]] constexpr bool operator>(const basic_dynamic_bitset& other) const
				noexcept(noexcept(std::declval<basic_dynamic_bitset>().operator<=(std::declval<const basic_dynamic_bitset&>())))
		{
			return not this->operator<=(other);
		}
		...
		*/
						...
						noexcept(bit_trait::block_index(std::declval<size_type>())))
		{
			...
			utils::gal_assert((last_block * bits_of_type) < static_cast<size_type>(std::numeric_limits<To>::digits));
		 ...
		 * @return result
		 */
		[[nodiscard]] constexpr bool is_subset_of(const basic_dynamic_bitset& other) const
				noexcept(
						noexcept(std::declval<container>()[0]))
		{
			utils::gal_assert(size() == other.size(), "container's size not equal to other's");
		 ...
		 * @return result
		 */
		[[nodiscard]] constexpr bool is_proper_subset_of(const basic_dynamic_bitset& other) const
				noexcept(
						noexcept(std::declval<container>()[0]))
		{
			utils::gal_assert(size() == other.size(), "container's size not equal to other's");
			utils::gal_assert(size() == other.size(), "container's container_size not equal to other's");
...
{
	template<>
	struct iterator_traits<gal::toolbox::container::basic_dynamic_bitset::bit_iterator>
