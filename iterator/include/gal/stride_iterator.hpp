	...
	public:
		...
		*/
		constexpr explicit stride_iterator(iterator_type iterator, difference_type count)
			...
			utils::gal_assert(count >= 0, "stride_iterator requires non-negative length n");
		}
		...
		*/
		...
		{
			utils::gal_assert(count_ >= 0, "stride_iterator dereference beyond end of range");
			return *base_;
		}
		...
		{
			utils::gal_assert(n < count_, "stride_iterator index out of range");
		...
		{
			utils::gal_assert(count_ > 0, "stride_iterator increment beyond end of range");
			move_base(1);
			return *this;
		}
		...
		constexpr stride_iterator& operator+=(difference_type n)
				noexcept(noexcept(move_base(std::declval<difference_type>())))
						requires std::random_access_iterator<iterator_type>
		{
			utils::gal_assert(n <= count_, "stride_iterator seek beyond end of range");
			move_base(n);
			return *this;
		}
		...
		constexpr stride_iterator& operator-=(difference_type n)
				noexcept(noexcept(std::declval<iterator_type>() -= 1))
						requires std::random_access_iterator<iterator_type>
		{
			utils::gal_assert(-n <= count_, "stride_iterator decrement beyond end of range");
			move_base(-n);
			return *this;
		}
