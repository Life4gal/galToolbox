	...
	*/
	...
	public:
		...
		*/
		 ...
		 * @param count count
		*/
		constexpr explicit stride_iterator(iterator_type iterator,
		                                   difference_type count)
		noexcept(std::is_nothrow_move_constructible_v<iterator_type>)
			: base_(std::move(iterator)),
			  count_(count) { gal_assert(count >= 0, "stride_iterator requires non-negative length n"); }
		...
		*/
		 ...
		 */
		constexpr const iterator_type& base() const & noexcept { return base_; }

		/**
		 * @brief get the underlying iterator
		 */
		[[nodiscard]] constexpr iterator_type base() && noexcept(std::is_nothrow_move_constructible_v<iterator_type>) { return std::move(base_); }

		/**
		 * @brief get the count
		 */
		[[nodiscard]] constexpr difference_type count() const noexcept { return count_; }

		[[nodiscard]] constexpr decltype(auto) operator*()
		noexcept(noexcept(*std::declval<iterator_type>()))
		{
			gal_assert(count_ >= 0, "stride_iterator dereference beyond end of range");
			return *base_;
		}
		...
		{
			gal_assert(n < count_, "stride_iterator index out of range");
			return base_[n * stride];
		}

		// stl compatible, can also be used for for-loop
		[[nodiscard]] constexpr stride_iterator begin() const noexcept { return {*this}; }
		...
		{
			gal_assert(count_ > 0, "stride_iterator increment beyond end of range");
			move_base(1);
			return *this;
		}

		[[nodiscard]] constexpr stride_iterator operator++(int)
		noexcept(noexcept(move_base(std::declval<difference_type>())))
		...
		noexcept(noexcept(std::declval<iterator_type>() + std::declval<difference_type>()))
			requires std::random_access_iterator<iterator_type> { return stride_iterator{base_ + n, static_cast<difference_type>(count_ - n)}; }

		[[nodiscard]] constexpr friend stride_iterator operator+(difference_type n,
		                                                         const stride_iterator& it)
		noexcept(noexcept(std::declval<iterator_type>() + std::declval<difference_type>()))
			requires std::random_access_iterator<iterator_type> { return stride_iterator{it.base_ + n, static_cast<difference_type>(it.count_ - n)}; }

		constexpr stride_iterator& operator+=(difference_type n)
		noexcept(noexcept(move_base(std::declval<difference_type>())))
			requires std::random_access_iterator<iterator_type>
		{
			gal_assert(n <= count_, "stride_iterator seek beyond end of range");
			move_base(n);
			return *this;
		}
		...
		[[nodiscard]] constexpr friend difference_type operator-(
				...
				) noexcept { return rhs.count() - lhs.count_; }

		[[nodiscard]] constexpr friend difference_type operator-(
				const stride_iterator& lhs,
				std::default_sentinel_t) noexcept { return -lhs.count_; }

		[[nodiscard]] constexpr friend difference_type operator-(
				std::default_sentinel_t,
				const stride_iterator& rhs
				) noexcept { return rhs.count_; }

		constexpr stride_iterator& operator-=(difference_type n)
		noexcept(noexcept(std::declval<iterator_type>() -= 1))
			requires std::random_access_iterator<iterator_type>
		{
			gal_assert(-n <= count_, "stride_iterator decrement beyond end of range");
			move_base(-n);
			return *this;
		}
		...
		[[nodiscard]] constexpr friend bool operator==(
				const stride_iterator& lhs,
				const stride_iterator<stride, I>& rhs
				) noexcept { return lhs.count_ == rhs.count(); }

		[[nodiscard]] constexpr friend bool operator==(
				const stride_iterator& lhs,
				std::default_sentinel_t) noexcept { return lhs.count_ == 0; }
	...
	public:
		...
		noexcept(noexcept(std::ranges::iter_move(other.base_)))
			requires std::input_iterator<iterator_type> { return std::ranges::iter_move(other.base_); }

		template<std::indirectly_swappable<iterator_type> I>
		constexpr friend void iter_swap(const stride_iterator& lhs,
		                                const stride_iterator<stride, I>& rhs)
		noexcept(noexcept(std::ranges::iter_swap(lhs.base_, rhs.base()))) { return std::ranges::iter_swap(lhs.base_, rhs.base()); }
	};
	...
	noexcept(std::is_nothrow_constructible_v<
		...
		Iterator,
		typename stride_iterator<Stride, Iterator>::difference_type>) { return stride_iterator<Stride, Iterator>{iterator, count}; }
}

namespace std
{
	template<std::size_t Stride, typename Iterator>
	struct incrementable_traits<gal::toolbox::iterator::stride_iterator<Stride, Iterator>>
	{
		using difference_type = typename gal::toolbox::iterator::stride_iterator<Stride, Iterator>::difference_type;
	};

	template<std::size_t Stride, std::input_iterator Iterator>
	struct iterator_traits<gal::toolbox::iterator::stride_iterator<Stride, Iterator>> : iterator_traits<Iterator>
	{
		using pointer = void;
	};

	template<std::size_t Stride, contiguous_iterator Iterator>
	struct pointer_traits<gal::toolbox::iterator::stride_iterator<Stride, Iterator>>
	{
		using pointer = gal::toolbox::iterator::stride_iterator<Stride, Iterator>;
		using element_type = remove_reference_t<typename gal::toolbox::iterator::stride_iterator<Stride, Iterator>::reference>;
		using difference_type = typename gal::toolbox::iterator::stride_iterator<Stride, Iterator>::difference_type;

		[[nodiscard]] constexpr static element_type* to_address(const pointer iterator) noexcept { return std::to_address(iterator.base()); }
	};
}
