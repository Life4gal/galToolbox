...
#include <galToolbox/utils/concepts.hpp>
...
namespace gal::toolbox::math
{
	...
	inline namespace utils
	{
		...
		[[nodiscard]] constexpr vector GAL_MATH_CALLCONV vector_load(const T* source) noexcept;
		...
		[[nodiscard]] constexpr matrix GAL_MATH_CALLCONV matrix_load(const T& source) noexcept;
		...
		template<std::size_t Index, basic_math_type_t ActiveType = float>
			requires(Index >= 0 && Index <= 3)
		constexpr void GAL_MATH_CALLCONV vector_get(ActiveType* dest, vector_f v) noexcept;

		/**
		 * @brief Sets a component of a vector(a copy) to a passed ActiveType value and return it
		 * @tparam Index x/y/z/w -> 0/1/2/3
		 * @tparam ActiveType active union member type
		 * @return vector
		 */
		...
		/**
		 * @brief Sets a component of a vector to an ActiveType value passed by pointer and return it
		 * @tparam Index x/y/z/w -> 0/1/2/3
		 * @tparam ActiveType active union member type
		 * @return vector
		 */
		template<std::size_t Index, basic_math_type_t ActiveType = float>
			requires(Index >= 0 && Index <= 3)
		[[nodiscard]] constexpr vector GAL_MATH_CALLCONV vector_set(vector_f v, const ActiveType* value) noexcept;

		/**
		 * @brief Get a vector, the four elements of which are determined by index0/index1/index2/index3, which respectively refer to which element of the parameter vector
		 * @note index must less than 4 (0/1/2/3 of the vector)
		 * @tparam ActiveType active union member type
		 * @return vector
		 */
		template<basic_math_type_t ActiveType = float>
		...
		/**
		 * @brief Get a vector, the four elements of which are determined by index0/index1/index2/index3, which respectively refer to which element of the two parameter vectors
		 * @note index must less than 8 (0/1/2/3 of the first vector, 4/5/6/7 of the second vector)
		 * @tparam ActiveType active union member type
		 * @return vector
		 */
		template<basic_math_type_t ActiveType = float>
		...
		/**
		 * @brief Define a control vector to be used in vector_select operations.
		 * @note The four integers specified in vector_select_control serve as indices to select between components in two vectors.
		 * A value of `0` for an index causes the corresponding component from the first vector to be selected whereas a `1` causes the component from the second vector to be selected instead.
		 * @example (0, 1, 0, 1) -> vector{ v1[0], v2[1], v1[2], v2[3] }
		 * @tparam ActiveType active union member type
		 * @return vector
		 */
		template<basic_math_type_t ActiveType = float>
		...
		/**
		 * @brief like vector_permute, but the indices are generated by vector_select_control
		 * @note can only decide whether each element chooses v1 or v2, and cannot control the choice arbitrarily
		 * @tparam ActiveType active union member type
		 * @return vector
		 */
		template<basic_math_type_t ActiveType = float>
	...
	inline namespace data_types
	{
		...
		struct alignas(math_type_alignment) generic_vector
		{
			...
			constexpr explicit operator const value_type*() const noexcept { return data.v1; }
				...
				requires basic_math_type_t<value_type>
			{
				...
				if (std::is_constant_evaluated())
				{
					vector v{};
					value_type* p;
					if constexpr (std::is_same_v<float, value_type>) { p = v.m128_f32; }
					else if constexpr (std::is_same_v<std::int32_t, value_type>) { p = v.m128_i32; }
					else { p = v.m128_u32; }
			...
			[[nodiscard]] inline explicit operator __m128i() const noexcept { return _mm_castps_si128(this->operator vector()); }

			[[nodiscard]] inline explicit operator __m128d() const noexcept { return _mm_castps_pd(this->operator vector()); }
		};
		...
		struct alignas(math_type_alignment) matrix
		{
			...
			[[nodiscard]] GAL_ASSERT_CONSTEXPR reference operator[](const size_type index) noexcept
			{
				gal_assert(index < extent_size, "index out of bound");

				return data[index];
			}

			[[nodiscard]] GAL_ASSERT_CONSTEXPR vector_type operator[](const size_type index) const noexcept
			{
				gal_assert(index < extent_size, "index out of bound");
		...
		struct generic_one_tier_container
		{
			...
			{
				gal_assert(index < size, "index out of bound");
			...
			[[nodiscard]] constexpr reference x() noexcept
				requires(size >= index_of_x + 1) { return data[index_of_x]; }
			...
			[[nodiscard]] constexpr reference y() noexcept
				requires(size >= index_of_y + 1) { return data[index_of_y]; }
			...
			[[nodiscard]] constexpr reference z() noexcept
				requires(size >= index_of_z + 1) { return data[index_of_z]; }
			...
			[[nodiscard]] constexpr reference w() noexcept
				requires(size >= index_of_w + 1) { return data[index_of_w]; }
		...
		struct generic_two_tier_container
		{
			...
			GAL_ASSERT_CONSTEXPR reference operator()(size_type row, size_type column) noexcept
			{
				gal_assert(row < first_size, "row out of bound");
				gal_assert(column < second_size, "row out of bound");

				return data[row][column];
			}

			GAL_ASSERT_CONSTEXPR value_type operator()(size_type row, size_type column) const noexcept
			{
				gal_assert(row < first_size, "row out of bound");
				gal_assert(column < second_size, "row out of bound");
		...
		struct alignas(math_type_alignment) float3x3a : float3x3 { };

		struct alignas(math_type_alignment) float3x4a : float3x4 { };

		struct alignas(math_type_alignment) float4x3a : float4x3 { };

		struct alignas(math_type_alignment) float4x4a : float4x4 { };
		...
		template<typename Target, typename... Args>
			requires(vector_or_matrix_t<Target> || one_or_two_tier_container_ignore_aligned_t<Target>) && (sizeof...(Args) <= Target::size)
		constexpr Target make(Args ... args) noexcept { return {static_cast<typename Target::value_type>(args)...}; }
	}// namespace data_types
...
#include <galToolbox/math/details/math_utils.inl>
#include <galToolbox/math/details/math_vector.hpp>
