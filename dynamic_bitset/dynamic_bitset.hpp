...
namespace gal::toolbox::dynamic_bitset {
	namespace detail {
		...
		requires std::random_access_iterator<typename Container<T, Allocator>::iterator> &&
				requires(typename Container<T, Allocator>::iterator it) {
			typename Container<T, Allocator>::iterator::iterator_category;
			typename Container<T, Allocator>::iterator::difference_type;
			it.operator*();
		}
		class bit_it;
		...
		public:
			...
			using iterator_type = typename Container<T, Allocator>::iterator;
	...
	public:
		 ...
		 * @param alloc the allocator of container
		 */
		constexpr explicit basic_dynamic_bitset(size_type size, value_type value = {}, const allocator_type& alloc = {}) noexcept(std::is_nothrow_constructible_v<container_type, allocator_type>&& noexcept(std::declval<basic_dynamic_bitset>().init_from_value_type(size, value)))
			: container(alloc),
			  total_size() {
		...
		constexpr basic_dynamic_bitset(ContainerInputIterator first, ContainerInputIterator last, const allocator_type& alloc = {}) noexcept(std::is_nothrow_constructible_v<container_type, allocator_type>&& noexcept(std::declval<basic_dynamic_bitset>().append_from_range(std::declval<basic_dynamic_bitset>().container.begin(), first, last)))
			: container(alloc),
			  total_size() {
		...
		void append_from_range(ContainerInputIterator pos, ContainerInputIterator first, ContainerInputIterator end) noexcept(
				noexcept(std::declval<basic_dynamic_bitset>().container.insert(pos, first, end)) && noexcept(std::declval<basic_dynamic_bitset>().container.size())) {
		...
		constexpr basic_dynamic_bitset(std::initializer_list<value_type>&& args, const allocator_type& alloc = {}) noexcept(
			...
			: container(alloc),
			  total_size() {
			container.insert(container.begin(), args.begin(), args.end());
			total_size += container.size() * bits_of_type;
		}
	...
	public:
		/**
		 * @brief Set blocks between pos and pos+len (dependent value)
		 * @param pos begin pos
		 * @param len length
		 * @param value value
		 * @return self
		 */
		constexpr basic_dynamic_bitset& set(size_type pos, size_type len, bool value) noexcept(
		...
		/**
		 * @brief Set a bit's value
		 * @param pos pos of bit
		 * @param value value
		 * @return self
		 */
		constexpr basic_dynamic_bitset& set(size_type pos, bool value = true) noexcept(
		...
		/**
		 * @brief Reset blocks between pos and pos+len (dependent value)
		 * @param pos begin pos
		 * @param len length
		 * @return self
		 */
		...
		[[nodiscard("If you do not receive the return value of dynamic_bitset::test, this call is meaningless overhead")]] constexpr bool test(size_type pos) const
				noexcept(
						noexcept(std::declval<basic_dynamic_bitset>().unchecked_test(pos))) {
			GAL_ASSERT(pos < total_size);
		 ...
		 * @return ref
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::operator[], this call is meaningless overhead")]] constexpr reference_type operator[](size_type index) const noexcept(std::is_nothrow_constructible_v<reference_type, value_type&, size_type>) {
			return {container[block_index(index)], bit_index(index)};
		}
		 ...
		 */
		constexpr void append(value_type value) noexcept(
		 ...
		 * @return copy
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::operator<<, this call is meaningless overhead")]] constexpr basic_dynamic_bitset operator<<(size_type n) const noexcept(noexcept(std::declval<basic_dynamic_bitset>().operator<<=(n))) {
			auto copy{*this};
			copy <<= n;
			return copy;
		}
		 ...
		 * @return copy
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::operator~, this call is meaningless overhead")]] constexpr basic_dynamic_bitset operator~() const noexcept(noexcept(std::declval<basic_dynamic_bitset>().flip())) {
			auto copy{*this};
			copy.flip();
			return copy;
		}
		...
		[[nodiscard("If you do not receive the return value of dynamic_bitset::operator<, this call is meaningless overhead")]] constexpr bool operator<(const basic_dynamic_bitset& other) const
				noexcept(noexcept(std::declval<basic_dynamic_bitset>().empty()) && noexcept(std::declval<basic_dynamic_bitset>().size()) && noexcept(std::declval<basic_dynamic_bitset>().container_size()) && noexcept(std::declval<basic_dynamic_bitset>().container[0])) {
			if (other.empty()) {
				return false;
			} else if (empty()) {
				return true;
		 ...
		 * @return result
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::operator<=, this call is meaningless overhead")]] constexpr bool operator<=(const basic_dynamic_bitset& other) const noexcept(noexcept(std::declval<basic_dynamic_bitset>().operator==(other)) && noexcept(std::declval<basic_dynamic_bitset>().operator<(other))) {
		 ...
		 * @return result
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::operator>, this call is meaningless overhead")]] constexpr bool operator>(const basic_dynamic_bitset& other) const noexcept(noexcept(std::declval<basic_dynamic_bitset>().operator<=(other))) {
			return !operator<=(other);
		}
		...
		requires(sizeof(To) >= sizeof(T))
				[[nodiscard("If you do not receive the return value of dynamic_bitset::cast_to, this call is meaningless overhead")]] constexpr To cast_to() const noexcept(noexcept(std::declval<basic_dynamic_bitset>().empty()) && noexcept(detail::bits_of_type<To>()) && noexcept(std::declval<basic_dynamic_bitset>().block_index(0))) {
			if (empty()) {
				return {};
			}
		...
		[[nodiscard("If you do not receive the return value of dynamic_bitset::is_subset_of, this call is meaningless overhead")]] constexpr bool is_subset_of(const basic_dynamic_bitset& other) const
				noexcept(noexcept(std::declval<basic_dynamic_bitset>().container_size()) && noexcept(std::declval<basic_dynamic_bitset>().container[0])) {
			GAL_ASSERT(size() == other.size());
		 ...
		 * @return result
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::is_intersects, this call is meaningless overhead")]] constexpr bool is_intersects(const basic_dynamic_bitset& other) const noexcept(noexcept(std::declval<basic_dynamic_bitset>().container_size()) && noexcept(std::declval<basic_dynamic_bitset>().container[0])) {
	...
	private:
		 ...
		 * @return ~0
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::set_block_full, this call is meaningless overhead")]] constexpr static value_type set_block_full(/* value_type_ref_if_cheaper block */) noexcept {
		 ...
		 * @return 0
		 */
		[[nodiscard("If you do not receive the return value of dynamic_bitset::reset_block_full, this call is meaningless overhead")]] constexpr static value_type reset_block_full(/* value_type_ref_if_cheaper block */) noexcept {
			return 0;
		}
		/**
		 ...
		 * @return self
		 */
		constexpr basic_dynamic_bitset& range_operation(size_type pos, size_type len,
														value_type (*partial_block_operation)(value_type, size_type, size_type),
														value_type (*full_block_operation)(value_type)) noexcept(noexcept(std::declval<basic_dynamic_bitset>().block_index(0)) && noexcept(std::declval<basic_dynamic_bitset>().bit_index(0)) && noexcept(std::declval<basic_dynamic_bitset>().container[0] = {}) &&
	...
	template<typename Char, typename Trait, typename T, typename Allocator, template<typename, typename> typename Container>
...
#ifdef GAL_DYNAMIC_BITSET_CLEAR_BEFORE_READ_FROM_ISTREAM
			bitset.clear();
#endif
			auto buf = is.rdbuf();
			auto i = buf->sgetc();
